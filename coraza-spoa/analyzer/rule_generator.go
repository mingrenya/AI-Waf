...
// MicroRuleDeployer 微规则部署接口。实现方调用引擎热加载新规则。
type MicroRuleDeployer interface {
	LoadRulesFromMongoDB() error
}
...
type RuleGenerator struct {
	...
	// microRuleDeployer 可选，注入后 DeployRule 会触发 MicroEngine 热加载
	microRuleDeployer MicroRuleDeployer
}
...
// SetMicroRuleDeployer 注入微规则部署器（引擎实现 MicroRuleDeployer 接口后调用此方法）。
func (rg *RuleGenerator) SetMicroRuleDeployer(deployer MicroRuleDeployer) {
	rg.microRuleDeployer = deployer
}
...
func (rg *RuleGenerator) generateModSecurityRule(pattern *model.AttackPattern) *model.GeneratedRule {
	...
	directive.WriteString("SecRule REQUEST_URI|ARGS|REQUEST_HEADERS ")
...
func (rg *RuleGenerator) DeployRule(ruleID string) error {
	...
	case "modsecurity":
		// ModSecurity (Coraza) 规则通过热重载 WAF 配置文件生效。
		// 当前仅更新 MongoDB 状态；Coraza 引擎将在下次配置重载时自动拉取 SecLang 指令。
		deployedRuleID = fmt.Sprintf("modsec_%s", objID.Hex())
		rg.logger.Infof("部署ModSecurity规则: %s", rule.SecLangDirective)

	case "micro_rule":
		// 先更新 MongoDB 状态，再通知 MicroEngine 热加载。
		deployedRuleID = fmt.Sprintf("micro_%s", objID.Hex())
		rg.logger.Infof("部署MicroRule规则: %s", rule.Name)
		if rg.microRuleDeployer != nil {
			if err := rg.microRuleDeployer.LoadRulesFromMongoDB(); err != nil {
				rg.logger.Errorf("MicroRule热加载失败: %v", err)
				return fmt.Errorf("MicroRule热加载失败: %w", err)
			}
			rg.logger.Infof("MicroRule热加载成功: %s", deployedRuleID)
		} else {
			rg.logger.Warnf("未注入 MicroRuleDeployer，规则 %s 已写入 MongoDB 但引擎未热加载", deployedRuleID)
		}
