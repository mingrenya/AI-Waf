			...
			if ctr, ok := req.(*mcp.CallToolRequest); ok {
				if len(ctr.Params.Arguments) > 0 {
					...
					if err := json.Unmarshal(ctr.Params.Arguments, &args); err == nil {
						...
						parentMessageUUID := extractParentMessageUUID(args)
						filtered := filterMCPMetadata(args)

						// 将过滤后的参数重新序列化
						if filteredJSON, err := json.Marshal(filtered); err == nil {
							ctr.Params.Arguments = filteredJSON
						}
